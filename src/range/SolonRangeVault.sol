...
// Fork of Beefy's BeefyVaultConcLiq (beefyfinance/beefy-zk, MIT). Ported changes are limited to:
// (1) de-proxied — plain constructor + one-shot setStrategy instead of initializer/upgradeable base,
// (2) OpenZeppelin v4 non-upgradeable bases. All share accounting, sliding-fee and flow logic is
 ...
 */
contract SolonRangeVault is ERC20Permit, Ownable, ReentrancyGuard {
    ...
    /// @notice One-shot wiring of the strategy (deploy order: vault → strategy(vault) → setStrategy).
    /// Replaces the upstream initializer; immutable thereafter.
    function setStrategy(address _strategy) external onlyOwner {
        if (address(strategy) != address(0)) revert StrategyAlreadySet();
        if (IStrategyVaultView(_strategy).vault() != address(this)) revert StrategyVaultMismatch();
    ...
    */
    ...
    function deposit(uint256 _amount0, uint256 _amount1, uint256 _minShares) public nonReentrant {
        ...
        if (amount0 > 0) IERC20(token0).safeTransferFrom(msg.sender, address(strategy), amount0);
        if (amount1 > 0) IERC20(token1).safeTransferFrom(msg.sender, address(strategy), amount1);
