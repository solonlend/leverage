...
// Fork of Beefy's StrategyPassiveManagerUniswap (beefyfinance/beefy-zk, MIT). Ported changes:
// (1) de-proxied — constructor wiring via RangeStratManager, deployed as a full immutable instance;
// (2) fee distribution is in-kind — the performance fee is paid out in lpToken0/lpToken1 directly
//     to the caller and treasury, removing the swapper/router/quoter/native dependency entirely
//     (_chargeFees is the only core function whose body changed: branches deleted, none added);
// (3) rebalancers/global pause/fees read from our RangeFactory.
// All position math, calm checks, sliding-fee wiring, locked-profit streaming and flow logic are
// verbatim upstream; see refs/beefy-zk-clm/ for the pristine source this is diffed against.

import {IERC20Metadata} from "@openzeppelin/contracts/token/ERC20/extensions/IERC20Metadata.sol";
import {SafeERC20} from "@openzeppelin/contracts/token/ERC20/utils/SafeERC20.sol";
import {SignedMath} from "@openzeppelin/contracts/utils/math/SignedMath.sol";
import {RangeStratManager} from "./RangeStratManager.sol";
import {IUniswapV3Pool} from "./interfaces/IUniswapV3Pool.sol";
import {LiquidityAmounts} from "./utils/LiquidityAmounts.sol";
import {TickMath} from "./utils/TickMath.sol";
import {TickUtils, FullMath} from "./utils/TickUtils.sol";
import {IStrategyConcLiq} from "./interfaces/IStrategyConcLiq.sol";
import {IERC20} from "@openzeppelin/contracts/token/ERC20/IERC20.sol";

/// @title Solon Range Strategy: Uniswap V3 passive concentrated-liquidity manager
/// @notice Fork of Beefy's passive position manager for Uniswap V3.
contract RangeStrategyUniV3 is RangeStratManager, IStrategyConcLiq {
    ...
    event ChargedFees(uint256 callFeeAmount0, uint256 callFeeAmount1, uint256 treasuryFeeAmount0, uint256 treasuryFeeAmount1);
    ...
    modifier onlyRebalancers() {
        if (!factory.rebalancers(msg.sender)) revert NotAuthorized();
        _;
    }
    ...
    /**
     * @notice Constructor replaces the upstream initializer; the factory deploys this as a full
     * immutable instance and wires the vault/factory linkage.
     * @param _pool The underlying Uniswap V3 pool.
     * @param _positionWidth The multiplier for tick spacing to find our range.
     * @param _vault The vault this strategy serves.
     * @param _factory The RangeFactory (ops registry).
     */
    constructor(
        address _pool,
        ...
        address _vault,
        address _factory
    ) RangeStratManager(_vault, _factory) {
        pool = _pool;
    ...
    function withdraw(uint256 _amount0, uint256 _amount1) external {
        ...
        // Fork change (2026-09-09): upstream gates the re-add on calm, which makes WITHDRAWALS
        // revert during price turmoil — trapping users exactly when they most want out. Here a
        // not-calm withdraw skips the re-add instead: remaining funds idle in the strategy and are
        // re-deployed by the next calm deposit/harvest/moveTicks. Exit is always available.
    ...
    function _addLiquidity() private onlyCalmPeriods {
        ...
        if (liquidity > 0 && amountsOk) {
            minting = true;
            IUniswapV3Pool(pool).mint(address(this), positionMain.tickLower, positionMain.tickUpper, liquidity, "Solon Main");
        ...
        if (liquidity > 0) {
            minting = true;
            IUniswapV3Pool(pool).mint(address(this), positionAlt.tickLower, positionAlt.tickUpper, liquidity, "Solon Alt");
        }
    }
    ...
    /// @notice Harvest call to claim fees from pool, charge fees, then readjust our positions.
    ...
    /// @notice Harvest call to claim fees from the pool, charge fees, then readjust our positions.
    /// @dev Call fee goes to the tx.origin.
    function harvest() external {
        _harvest(tx.origin);
    }

    /// @notice Internal function to claim fees from the pool, charge fees, then readjust our positions.
    function _harvest(address _callFeeRecipient) private onlyCalmPeriods {
        // Claim fees from the pool and collect them.
        _claimEarnings();
        _removeLiquidity();

        // Charge fees and send them to the appropriate addresses, charge fees to accrued state fee amounts.
    ...
    /**
     * @notice Internal function to charge fees and send them to the appropriate addresses.
     * @dev In-kind distribution (fork change): the performance fee is taken in lpToken0/lpToken1
     * directly — caller tip to _callFeeRecipient, remainder to the treasury. No swaps.
     * @param _callFeeRecipient The address to send the call fee to.
    ...
    function _chargeFees(address _callFeeRecipient, uint256 _amount0, uint256 _amount1) private returns (uint256 _amountLeft0, uint256 _amountLeft1) {
        /// Fetch our fee percentage amounts from the factory.
        (uint256 feeTotal, uint256 feeCall) = getFees();

        uint256 fee0 = _amount0 * feeTotal / DIVISOR;
        uint256 fee1 = _amount1 * feeTotal / DIVISOR;
        _amountLeft0 = _amount0 - fee0;
        _amountLeft1 = _amount1 - fee1;

        uint256 callFeeAmount0 = fee0 * feeCall / DIVISOR;
        uint256 callFeeAmount1 = fee1 * feeCall / DIVISOR;
        if (callFeeAmount0 > 0) IERC20Metadata(lpToken0).safeTransfer(_callFeeRecipient, callFeeAmount0);
        if (callFeeAmount1 > 0) IERC20Metadata(lpToken1).safeTransfer(_callFeeRecipient, callFeeAmount1);

        uint256 treasuryFeeAmount0 = fee0 - callFeeAmount0;
        uint256 treasuryFeeAmount1 = fee1 - callFeeAmount1;
        if (treasuryFeeAmount0 > 0) IERC20Metadata(lpToken0).safeTransfer(treasury(), treasuryFeeAmount0);
        if (treasuryFeeAmount1 > 0) IERC20Metadata(lpToken1).safeTransfer(treasury(), treasuryFeeAmount1);

        emit ChargedFees(callFeeAmount0, callFeeAmount1, treasuryFeeAmount0, treasuryFeeAmount1);
    }
    ...
    */
    ...
    function _setAltTick(int24 tick, int24 distance, int24 width) private {
        ...
        if (amount0 < bal1) {
            ...
            // Fork change (2026-09-09, RT-1): upstream uses `else if (bal1 < amount0)`, leaving the
            // alt ticks UNSET (lower==upper) when both sides are exactly value-equal — the liquidity
            // math then divides by zero and bricks the deposit. The balanced case now deterministically
            // takes the token0-side branch; with zero balances the resulting empty alt range simply
            // mints nothing.
            (, positionAlt.tickLower) = TickUtils.baseTicks(
                tick,
                distance,
    ...
    */
    function setDeviation(int56 _maxDeviation) external onlyOwner {
        ...
        // Fork change (2026-09-09): the upstream bound (< tickSpacing * 4) assumes a 1bp fee tier
        // implies a stable pair. RH's flagship WETH/USDG pool is a volatile pair at 1bp (measured
        // 120s TWAP moves: median 2.7 / p90 8.9 / max 11 ticks over a sample hour), so a 3-tick cap
        // would false-trigger NotCalm on normal drift. Absolute bound instead: 0 < dev ≤ 200 ticks (~2%).
        if (_maxDeviation <= 0 || _maxDeviation > 200) revert InvalidInput();
    ...
    */
    ...
    /// @notice Retire the strategy and return all the dust to the treasury.
    function retireVault() external onlyOwner {
        if (IERC20(vault).totalSupply() != 10**3) revert NotAuthorized();
        panic(0,0);
    ...
    /**
     * @notice Remove Liquidity, then pause deposits.
    ...
    /// @notice Unpause deposits and add liquidity.
