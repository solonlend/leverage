...
contract LendingPool is ILendingPool, Ownable, Payments, ReentrancyGuard {
    ...
    /// @dev Gates operations that add exposure or reconfigure the pool while an
    /// emergency pause is active. Debt-reducing and wind-down operations do not
    /// use this modifier.
    modifier whenRiskIncreasingOperationsAllowed() {
        require(!paused, Errors.LP_IS_PAUSED);
        _;
    }

    function _requireNotPausedOrRiskReducing(
        bool riskReducing
    ) internal view {
        require(!paused || riskReducing, Errors.LP_IS_PAUSED);
    }
    ...
    function initReserve(address asset) external onlyOwner whenRiskIncreasingOperationsAllowed {
    ...
    function deposit(
        ...
        address onBehalfOf,
        uint16 referralCode
    ) public payable whenRiskIncreasingOperationsAllowed nonReentrant returns (uint256 eTokenAmount) {
        eTokenAmount = _deposit(reserveId, amount, onBehalfOf);

        // if there is unused ETH, refund it to _msgSender()
        if (msg.value > 0) {
            refundETH(
                msg.value - (reserves[reserveId].underlyingTokenAddress == WETH9 ? amount : 0)
            );
        }
    ...
    function depositAndStake(
        ...
        address onBehalfOf,
        uint16 referralCode
    ) external payable whenRiskIncreasingOperationsAllowed nonReentrant returns (uint256 eTokenAmount) {
    ...
    ) public whenRiskIncreasingOperationsAllowed nonReentrant returns (uint256) {
    ...
    function unStakeAndWithdraw(
        ...
        address to,
        bool receiveNativeETH
    ) external whenRiskIncreasingOperationsAllowed nonReentrant returns (uint256) {
        address stakingPool = reserves[reserveId].stakingAddress;
        require(stakingPool != address(0), "Address=0");
    ...
    function newDebtPosition(
        uint256 reserveId
    ) external whenRiskIncreasingOperationsAllowed nonReentrant returns (uint256 debtId) {
        DataTypes.ReserveData storage reserve = getReserve(reserveId);
        require(!reserve.getFrozen(), Errors.VL_RESERVE_FROZEN);
    ...
    function borrow(
        ...
        uint256 debtId,
        uint256 amount
    ) external whenRiskIncreasingOperationsAllowed nonReentrant {
        require(
    ...
    function enableVaultToBorrow(
        uint256 vaultId
    ) external onlyOwner whenRiskIncreasingOperationsAllowed {
    ...
    function setCreditsOfVault(
        ...
        address vaultAddr = getVault(vaultId);
        _requireNotPausedOrRiskReducing(
            credit <= credits[reserveId][vaultAddr]
        );
    ...
    function activateReserve(
        uint256 reserveId
    ) public onlyOwner whenRiskIncreasingOperationsAllowed {
        DataTypes.ReserveData storage reserve = reserves[reserveId];
    ...
    function freezeReserve(uint256 reserveId) public onlyOwner {
        DataTypes.ReserveData storage reserve = reserves[reserveId];
    ...
    function disableBorrowing(uint256 reserveId) public onlyOwner {
        DataTypes.ReserveData storage reserve = reserves[reserveId];
    ...
    function setReserveCapacity(
        ...
        DataTypes.ReserveData storage reserve = reserves[reserveId];
        _requireNotPausedOrRiskReducing(cap <= reserve.reserveCapacity);
